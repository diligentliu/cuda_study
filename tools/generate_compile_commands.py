...
query (e.g., for the example usage below `bazel build //cuda_study/...`). This is due
...
Example usage:
  bazel aquery "mnemonic(CppCompile, //cuda_study/...)" --output=jsonproto | \
...
_CUDA_STUDY_SRC_ROOT = pathlib.Path(__file__).absolute().parent.parent
...
def main():
  ...
  if not (external := _CUDA_STUDY_SRC_ROOT / "external").exists():
    logging.info("Symlinking `cuda_study/bazel-cuda_study/external` to `cuda_study/external`")
    external.symlink_to(_CUDA_STUDY_SRC_ROOT / "bazel-cuda_study" / "external")
  ...
  with (_CUDA_STUDY_SRC_ROOT / "compile_commands.json").open("w") as f:
    json.dump(
        [
            command.to_dumpable_json(directory=str(_CUDA_STUDY_SRC_ROOT))
